            ...
            <Reveal width="100%" delay={0.2}>
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-blue-900/5 p-6 md:p-8 h-full flex flex-col">
                ...
                <div className="flex justify-between items-center mb-8">
                  ...
                  <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full border border-slate-200">{songs.length} Nyimbo</span>
                </div>
                  ...
                  {songs.length === 0 ? (
                    ...
                    songs.map((song, index) => (
                      <div 
                        ...
                        className={`group flex items-center p-3 rounded-2xl cursor-pointer transition-all duration-300 border ${
                          currentSong?.id === song.id 
                            ? 'bg-blue-50 border-blue-100 shadow-sm translate-x-2' 
                      ...
                      >
                        ...
                        <div className="relative w-12 h-12 rounded-xl overflow-hidden mr-4 bg-slate-200 shrink-0">
                           {currentSong?.id === song.id && isPlaying && (
                             <div className="absolute inset-0 bg-blue-900/60 flex items-center justify-center z-10">
                               <BarChart3 size={16} className="text-white animate-pulse"/>
                             </div>
                           )}
                        ...
                        <div className="flex-1 min-w-0">
                          <h5 className={`text-sm font-bold truncate ${currentSong?.id === song.id ? 'text-blue-700' : 'text-slate-700 group-hover:text-slate-900'}`}>
                            {song.title}
                          </h5>
                          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">Audio Track</p>
                        </div>

                        <div className="text-slate-300">
                          {currentSong?.id === song.id ? <div className="w-2 h-2 bg-blue-600 rounded-full animate-ping"></div> : <Play size={16} className="group-hover:text-blue-600 transition-colors"/>}
                        </div>
                      </div>
                    ))
                  )}
