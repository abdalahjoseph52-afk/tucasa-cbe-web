...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((u) => {
      ...
      if (u) {
        ...
        const handleDataLoad = () => { /* Logic simplified */ }; 
  ...
  const renderFormContent = () => {
    switch (activeTab) {
      ...
      case 'gallery': return (<><input className="w-full p-4 border rounded-xl" placeholder="Caption" value={formData.alt||''} onChange={e=>setFormData({...formData, alt:e.target.value})}/><FileInput label="Photo" accept="image/*" onChange={setSelectedFile} file={selectedFile}/></>);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-900">
      ...
      {/* MOBILE NAV (Fixed Bottom) - Z-INDEX INCREASED */}
      ...
      {/* MAIN CONTENT - PADDING ADDED FOR MOBILE NAV */}
      <main className="flex-1 p-4 md:p-8 overflow-y-auto pb-32 md:pb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data[activeTab].map(item => (
              <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between relative group">
                ...
                {/* BUTTONS (FIXED Z-INDEX & TOUCH TARGETS) */}
        ...
        {isModalOpen && (
          // FIX: Z-INDEX 100 to cover everything, FIXED inset-0 for full screen
          <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in zoom-in duration-200" onClick={closeModal}>
            <div className="bg-white w-full h-full md:h-auto md:max-h-[85vh] md:max-w-lg md:rounded-3xl shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                <h3 className="font-extrabold text-xl text-slate-900 capitalize">{editingId ? 'Edit' : 'New'} {activeTab.slice(0, -1)}</h3>
                <button onClick={closeModal} className="p-2 bg-slate-100 hover:bg-red-100 hover:text-red-600 rounded-full transition-colors"><X size={24}/></button>
              </div>
              
              {/* Modal Body (Scrollable) */}
              <div className="p-4 md:p-6 overflow-y-auto flex-1 space-y-4 bg-slate-50/50">
                {renderFormContent()}
              </div>

              {/* Modal Footer (Action) */}
              <div className="p-4 md:p-6 border-t border-slate-100 bg-white sticky bottom-0 z-10 safe-area-pb">
                <button disabled={isSubmitting} onClick={() => handleSave(activeTab, activeTab === 'resources' ? 'link' : activeTab === 'gallery' ? 'src' : activeTab === 'songs' ? 'cover' : 'image', activeTab === 'resources' ? 'file' : 'image')} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">
                  {isSubmitting ? <><Loader2 className="animate-spin" size={24}/> Processing...</> : "Save Changes"}
                </button>
              </div>
